...
export default function RiskHistoryChart({ data = [], loading = false }) {
  ...
  const chartData = useMemo(() => {
    // Filter out invalid/NaN entries to prevent coordinate math errors
    const safeData = data.filter(d => typeof d.avgRisk === 'number' && !isNaN(d.avgRisk));
    
    if (!safeData.length) return { points: [], path: '', areaPath: '', yTicks: [], trend: 0 };

    const maxVal = Math.max(...safeData.map(d => d.avgRisk), 80);
    const minVal = Math.min(...safeData.map(d => d.avgRisk), 0);
    const range = maxVal - minVal || 1;

    const points = safeData.map((d, i) => ({
      x: padding.left + (i / Math.max(safeData.length - 1, 1)) * chartW,
