...
export function exportStudentsToXLSX(students, tutors = [], interventionsByStudent = {}) {
  if (!Array.isArray(students) || students.length === 0) {
    console.warn('[XLSX Export] No hay estudiantes para exportar.');
    return;
  }
  ...
  const filename = `edualert-export-${today}.xlsx`;
  ...
  console.log(`[XLSX Export] Exported ${students.length} students → ${filename}`);
}
